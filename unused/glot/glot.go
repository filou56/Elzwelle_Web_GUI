...
func NewPlot(dimensions int, persist, debug bool) (*Plot, error) {
	...
	p.proc = proc
	p.reader = proc.stdout
	return p, nil
}

func (plot *Plot) GetReader() io.ReadCloser {
	return plot.reader
}
...
func (plot *Plot) plotXY(PointGroup *PointGroup) error {
	...
	var line string
	if PointGroup.name == "" {
		line = fmt.Sprintf("%s \"%s\" using 1:2 with %s", cmd, fname, PointGroup.style) // FILOU using
	} else {
		line = fmt.Sprintf("%s \"%s\" using 1:2 with %s title \"%s\"", cmd, fname, PointGroup.style, PointGroup.name) // FILOU using
	}
	plot.nplots++
	log.Println(line)
	return plot.Cmd(line)
